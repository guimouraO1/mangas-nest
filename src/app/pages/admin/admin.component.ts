...
export class AdminComponent {
  ...
  teste2(index: number) {
    const currentRating = this.mangaForm.get('rating')?.value ?? 0; // Usa 0 se for undefined
    const newRating = index + currentRating;
    this.mangaForm.get('rating')?.setValue(newRating); // Atualiza o rating e garante que não exceda 5
  }
  ...
  // increment(): void {
  //   const currentValue: number = this.mangaForm.get('chapter')?.value || 0;
  //   this.mangaForm.get('chapter')?.setValue(currentValue + 1);
  // }
